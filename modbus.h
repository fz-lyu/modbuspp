...
public:
    ...
    int  modbus_write_coil(int address, bool to_write);
    int  modbus_write_register(int address, uint16_t value);
    int  modbus_write_coils(int address, int amount, const bool* value );
    int  modbus_write_registers(int address, int amount, uint16_t *value);
...
int modbus::modbus_write(int address, uint amount, int func, const uint16_t *value) {
	...
	if(func == WRITE_COIL || func == WRITE_REG) {
		...
		status = modbus_send(to_send, 14 + (amount - 1u) / 8u);
	}
	return status;
 ...
 */
int modbus::modbus_write_coil(int address, bool to_write) {
	if(_connected) {
		if(address > 65535) {
			set_bad_input();
			return EX_BAD_DATA;
 ...
 */
int modbus::modbus_write_register(int address, uint16_t value) {
	if(_connected) {
		if(address > 65535) {
			set_bad_input();
			return EX_BAD_DATA;
 ...
 */
int modbus::modbus_write_registers(int address, int amount, uint16_t *value) {
	if(_connected) {
		if(address > 65535 || amount > 65535) {
			set_bad_input();
			return EX_BAD_DATA;
